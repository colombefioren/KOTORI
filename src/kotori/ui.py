...
**playground** (the brief and the reader side by side, with the story written
out full width underneath) and **history** (every story ever kept, each one with
its own player).
"""
...
def build_app(studio: Studio | None = None, settings: Settings | None = None) -> gr.Blocks:
    ...
    with gr.Blocks(
        ...
        with gr.Column(elem_classes=["ast-shell"]):
            ...
            with gr.Tabs(elem_id="ast-tabs", selected="home") as tabs:
                # ── home ─────────────────────────────────────────────────────
                with gr.Tab("home", id="home"):
                    with gr.Column(elem_classes=["room", "home"]):
                        gr.HTML(render_home_intro(settings), elem_id="ast-home-intro")
                        with gr.Row(elem_classes=["home__cta"]):
                            start = gr.Button(
                                "start writing", variant="primary", elem_id="ast-start"
                            )
                ...
                with gr.Tab("playground", id="playground"):
                    with gr.Column(elem_classes=["room"]):
                        with gr.Row(elem_classes=["ast-grid"]):
                            with gr.Column(scale=6, elem_id="ast-composer"):
                                ...
                                with gr.Group(elem_classes=["card", "ast-stack"]):
                                    ...
                                    genre = gr.Dropdown(
                                        ...
                                        info="pick one, or paste your own",
                                    )
                                    with gr.Row():
                                        mood = gr.Dropdown(
                                            choices=MOOD_LABELS,
                ...
                with gr.Tab("history", id="history"):
                    with gr.Column(elem_classes=["room", "ledger"]):
        ...
        play_targets = [stage, deck, status, tabs]
        ...
        hear_demo.click(
            ...
            outputs=[*stage_targets, ignite, tabs],
            api_name="hear_demo",
            show_progress="hidden",
        )
